...
const Responsive_Coding_Assignment_1 = () => {
  ...
  const loadProgressData = useCallback(async () => {
    try {
      ...
      // Load completion status for all responsive practices
      const completionStatus = {};
      for (const practice of responsiveCodingPracticesData.responsive) {
        try {
  ...
  // Load practice
  useEffect(() => {
    const practice17 = responsiveCodingPracticesData.responsive.find(
      (p) => p.id === "responsive-coding-assignment-1"
    );
    if (practice17) {
      setSelectedPractice(practice17);
      console.log("✅ Loaded practice:", practice17.id);
    }
    setLoading(false);
...
export default Responsive_Coding_Assignment_1;
