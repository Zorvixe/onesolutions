...
const Login = () => {
  ...
  const { loginOtpRequest, loginOtpVerify, error, clearError, isAuthenticated, otpSent } = useAuth()
  ...
  const handleCredentialSubmit = async (e) => {
    ...
    try {
      ...
      const response = await loginOtpRequest(formData.email.trim(), formData.password)
  ...
  return (
    <div className="login-container-login">
      <div className="image-section-login">
        <img src="/assets/img/login_img.png" alt="Login Illustration" loading="lazy" />
      </div>

      <div className="form-section-login">
        <div className="card-login">
          <div className="login_log_con">
            <img src="/assets/img/login_logo.png" alt="OneSolutions Logo" className="login_logo" loading="lazy" />
          </div>

          <h2 className="title-login">Welcome Back, OneSolutions!</h2>
          <p className="para-login">Let's hustle and make your dreams come true!</p>
            ...
            <form onSubmit={handleCredentialSubmit} className="form-login">
              <div className="form-group-login">
                ...
                <input
                  ...
                  className={`input-login ${validationErrors.email ? "error-login" : ""}`}
                  placeholder="Enter your email"
                  disabled={isSubmitting}
                  autoComplete="email"
                  required
                />
                {validationErrors.email && <span className="error-text-login">{validationErrors.email}</span>}
              </div>
              ...
              <div className="form-group-login">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`input-login ${validationErrors.password ? "error-login" : ""}`}
                  placeholder="Enter your password"
                  disabled={isSubmitting}
                  autoComplete="current-password"
                  required
                />
                {validationErrors.password && <span className="error-text-login">{validationErrors.password}</span>}
            ...
            <form onSubmit={handleOtpSubmit} className="form-login">
              ...
              <div className="form-group-login">
                ...
                <input
                  ...
                  maxLength={6}
                  onChange={handleChange}
                  className={`input-login otp-input-login ${validationErrors.otp ? "error-login" : ""}`}
                  placeholder="000000"
                  disabled={isSubmitting}
                  autoComplete="off"
                  inputMode="numeric"
                  required
                />
                {validationErrors.otp && <span className="error-text-login">{validationErrors.otp}</span>}
              </div>

              <button type="submit" className="button-login" disabled={isSubmitting || !otp.trim() || otp.length !== 6}>
                {isSubmitting ? (
                  <>
              ...
              <p className="resend-otp-text">
                ...
                >
                  {otpTimer > 0 ? `Resend in ${otpTimer}s` : isSubmitting ? "Resending..." : "Resend OTP"}
                </button>
              </p>

              <div className="link-login">
                <button type="button" onClick={handleBackToLogin} className="back-btn-login" disabled={isSubmitting}>
                  ← Back to Login
                </button>
              </div>
            </form>
