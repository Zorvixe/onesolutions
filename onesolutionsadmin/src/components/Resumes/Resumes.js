    ...
    const fetchAll = async () => {
      ...
      try {
        const [pubResp, atsResp] = await Promise.all([
          ...
          fetch('https://backend-lt9m.onrender.com/api/analyzed-resumes')
        ]);
        ...
        <section className="resumes-section">
          ...
          {publicResumes.map((resume) => (
            <tr key={resume.id}>
                    ...
                    <td>
                     <a
                  href={`https://backend-lt9m.onrender.com/api/public/resumes/${resume.id}/download`}
              ...
              <tbody>
                {atsResumes.map(({ id, name, email, phone, match_percentage }) => (
                  <tr key={id}>
                    ...
                    <td>
                      <a
                        href={`https://backend-lt9m.onrender.com/api/analyzed-resumes/${id}/download`}
                        className="download-link"
                      >
                        📄 Download
                      </a>
