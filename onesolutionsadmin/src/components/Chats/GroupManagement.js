...
const GroupManagement = () => {
  ...
  const fetchGroups = async () => {
    try {
      const response = await fetch('https://backend-lt9m.onrender.com/api/chat/rooms');
  ...
  const handleUpdate = async () => {
    try {
      const response = await fetch(`https://backend-lt9m.onrender.com/api/chat/rooms/${editingGroup.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = async (groupId) => {
    try {
      const response = await fetch(`https://backend-lt9m.onrender.com/api/chat/rooms/${groupId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
