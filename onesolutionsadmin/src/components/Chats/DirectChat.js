...
const DirectChat = ({ recipient, user, socket, onBack }) => {
    ...
    const fetchMessages = async () => {
      try {
        ...
        const response = await axios.get(
          `https://backend-lt9m.onrender.com/api/chat/direct-messages/${user.phone}/${recipient.phone}`,
  ...
  const sendMessage = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        "https://backend-lt9m.onrender.com/api/chat/direct-messages",
        messagePayload,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setNewMessage("");
  ...
  const handleEditMessage = async (messageId, newText) => {
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `https://backend-lt9m.onrender.com/api/chat/direct-messages/${messageId}`,
        { message: newText },
        { headers: { Authorization: `Bearer ${token}` } }
      );
