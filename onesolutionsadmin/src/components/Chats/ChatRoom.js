...
const ChatRoom = ({ roomId, roomName, user, socket, onBack }) => {
    ...
    const fetchMessages = async () => {
      try {
        ...
        const response = await axios.get(
          `https://backend-lt9m.onrender.com/api/chat/messages/${roomId}`,
  ...
  const sendMessage = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        "https://backend-lt9m.onrender.com/api/chat/messages",
        message,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setNewMessage("");
  ...
  const handleEditMessage = async (messageId, newText) => {
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `https://backend-lt9m.onrender.com/api/chat/messages/${messageId}`,
        { message: newText },
        { headers: { Authorization: `Bearer ${token}` } }
      );
