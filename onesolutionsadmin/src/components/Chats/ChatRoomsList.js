...
const ChatRoomsList = ({ user, onSelectChat }) => {
    ...
    const fetchAdminDetails = async () => {
      ...
      try {
        const response = await fetch(
          "https://backend-lt9m.onrender.com/api/admin/me",
          {
            headers: {
  ...
  const fetchRooms = async () => {
    try {
      ...
      const response = await axios.get(
        "https://backend-lt9m.onrender.com/api/chat/rooms",
  ...
  const fetchAdmins = async () => {
    try {
      ...
      const [adminsResponse, statusResponse] = await Promise.all([
        axios.get("https://backend-lt9m.onrender.com/api/admins/approved", {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get("https://backend-lt9m.onrender.com/api/admins/status/individual", {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  const updateRoom = async (roomId, newName) => {
    try {
      ...
      const token = localStorage.getItem("token");
      await axios.put(
        `https://backend-lt9m.onrender.com/api/chat/rooms/${roomId}`,
        { room_name: newName },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const socket = io("https://backend-lt9m.onrender.com"); // Replace with your server URL
