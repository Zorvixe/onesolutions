...
const PendingAdmins = () => {
  ...
  const fetchPendingAdmins = async () => {
    try {
      ...
      const response = await axios.get(
        "https://backend-lt9m.onrender.com/api/admin/pending",
        {
          headers: {
  ...
  const approveAdmin = async (id) => {
    try {
      const response = await axios.put(
        `https://backend-lt9m.onrender.com/api/admin/approve/${id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const rejectAdmin = async (id) => {
    try {
      const response = await axios.put(
        `https://backend-lt9m.onrender.com/api/admin/reject/${id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
