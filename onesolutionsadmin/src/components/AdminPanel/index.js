...
const AdminPanel = () => {
  ...
  const fetchJobs = useCallback(
    async (token) => {
      setLoading(true)
      try {
        const url = `https://backend-lt9m.onrender.com/api/jobs/adminpanel${viewAllJobs ? "?view=all" : ""}`
  ...
  const performDelete = async () => {
    const token = localStorage.getItem("token")
    try {
      const response = await fetch(`https://backend-lt9m.onrender.com/api/jobs/${jobToDelete}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      })
      if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    const token = localStorage.getItem("token")
    const url = editJobId
      ? `https://backend-lt9m.onrender.com/api/jobs/${editJobId}`
      : `https://backend-lt9m.onrender.com/api/jobs`
